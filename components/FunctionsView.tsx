...
const FunctionsView: React.FC<FunctionsViewProps> = ({ functions, onSaveFunction }) => {
  ...
  return (
    <div className="flex-1 p-8 overflow-auto custom-scrollbar bg-slate-900/60 backdrop-blur-xl border-white/10 shadow-sm  relative">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h2 className="text-2xl font-bold text-slate-50 ">Functions & Private Events</h2>
          <p className="text-slate-400 ">Manage private bookings and large groups.</p>
        </div>
        <button 
      ...
      <div className="space-y-6">
        {functions.map(func => (
          <div key={func.id} className="bg-slate-900/60 backdrop-blur-xl border-white/10 shadow-sm  rounded-xl border border-white/10  overflow-hidden hover:border-indigo-300 dark:hover:border-indigo-700 transition-colors">
             <div className="p-6">
               <div className="flex flex-col md:flex-row justify-between md:items-center mb-4 gap-4">
                 <div className="flex items-center space-x-4">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-slate-400 ">Client: {func.clientName}</div>
                    </div>
                 </div>
                 <div className="flex items-center space-x-3">
                    <div className="text-right">
                       <div className="font-semibold text-slate-50 ">{formatDate(func.date)}</div>
                       <div className="text-sm text-slate-400">{formatTime(func.date)}</div>
                    </div>
               ...
               <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                  <div className="flex items-center p-3 bg-slate-900/60 backdrop-blur-xl border-white/10 shadow-sm /30 rounded-lg">
               ...
               {func.requirements.length > 0 && (
                   <div className="bg-indigo-50 dark:bg-indigo-900/10 rounded-lg p-4">
                      ...
                      <ul className="list-disc list-inside text-sm text-slate-200  grid grid-cols-1 md:grid-cols-2 gap-1">
             ...
             <div className="bg-slate-900/60 backdrop-blur-xl border-white/10 shadow-sm /50 px-6 py-3 border-t border-white/5  flex justify-end space-x-3">
      ...
      {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
              <div className="bg-slate-900/60 backdrop-blur-xl border-white/10 shadow-sm  rounded-2xl w-full max-w-lg p-6 shadow-xl animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto custom-scrollbar">
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                      ...
                      <div>
                          <label className="block text-sm font-medium text-slate-200  mb-1">Client Name</label>
                          <input 
                            type="text" 
                            required 
                            className="w-full px-3 py-2 bg-slate-900/60 backdrop-blur-xl border-white/10 shadow-sm  border border-white/10  rounded-lg outline-none"
                      ...
                      <div>
                          <label className="block text-sm font-medium text-slate-200  mb-1">Requirements</label>
                          <div className="flex gap-2 mb-2">
                              <input 
                                type="text" 
                                className="flex-1 px-3 py-2 bg-slate-900/60 backdrop-blur-xl border-white/10 shadow-sm  border border-white/10  rounded-lg outline-none"
                          ...
                          <div className="flex flex-wrap gap-2">
                              {newFunction.requirements?.map((req, i) => (
                                  <span key={i} className="px-2 py-1 bg-gray-100  rounded text-xs flex items-center">
                                      {req}
                                      <button 
                                        type="button" 
                      ...
                      <div className="flex items-center space-x-3 pt-2">
                          <input 
                            type="checkbox" 
                            id="deposit"
                            className="w-4 h-4 rounded border-white/10 text-indigo-600 focus:ring-indigo-500"
                            checked={newFunction.depositPaid}
                            onChange={e => setNewFunction({...newFunction, depositPaid: e.target.checked})}
                          />
                          <label htmlFor="deposit" className="text-sm text-slate-200 ">Deposit has been paid</label>
                      </div>

                      <div className="flex justify-end space-x-3 pt-6 border-t border-white/5  mt-6">
