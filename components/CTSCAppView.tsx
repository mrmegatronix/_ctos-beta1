      ...
      {activeTab === 'alerts' && (
        <div className="space-y-4">
          <div className="flex justify-between items-center mb-2">
            <h3 className="text-sm font-black uppercase tracking-wider text-slate-600 dark:text-slate-300 flex items-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {alerts.map(alt => (
              <div
                key={alt.id}
                className={`bg-white dark:bg-slate-800 border rounded-3xl p-5 shadow-sm transition-all flex flex-col justify-between ${
                  alt.status === 'pending'
              ...
              >
                <div>
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      <h4 className="text-lg font-black text-slate-900 dark:text-slate-50">{alt.tableNumber}</h4>
        ...
        <div className="space-y-6">
          <div className="flex justify-between items-center mb-2">
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700 text-xs font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors flex items-center space-x-1"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {kegs.map(keg => (
              <div
                key={keg.id}
                className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl p-6 shadow-sm flex flex-col justify-between"
              >
                <div>
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h4 className="font-black text-base text-slate-900 dark:text-slate-50">{keg.name}</h4>
                  ...
                  {/* Sensor Stats */}
                  <div className="grid grid-cols-2 gap-3 p-3 bg-slate-50 dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-700 text-xs">
                    <div>
                      <span className="text-slate-400 block text-[10px] uppercase font-bold">Line Temp</span>
                      <span className="font-black text-slate-800 dark:text-slate-100">{keg.tempCelsius}°C</span>
                    </div>
                    <div>
                      <span className="text-slate-400 block text-[10px] uppercase font-bold">CO2 Pressure</span>
                      <span className="font-black text-slate-800 dark:text-slate-100">{keg.psi} PSI</span>
                    </div>
                  </div>
                </div>
              ...
              <form onSubmit={handleSendBroadcast} className="space-y-4">
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full p-3.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl text-xs text-slate-800 dark:text-slate-100 outline-none focus:ring-2 focus:ring-indigo-500"
                  />
                </div>

                <div className="flex items-center space-x-2 p-3 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700">
                  <input
                    type="checkbox"
                    id="urgentCheck"
                    checked={isUrgent}
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center space-x-2">
                      <span className="font-bold text-xs text-slate-900 dark:text-slate-50">{bc.sender}</span>
                      {bc.priority === 'urgent' && (
      ...
      {activeTab === 'quickpos' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center space-x-2">
                ...
                <select
                  ...
                  className="px-3 py-1.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-bold text-slate-800 dark:text-slate-100 outline-none"
                >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {quickItems.map(item => (
                <button
                  ...
                  className="p-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl text-left hover:border-indigo-500 hover:shadow-md transition-all flex flex-col justify-between min-h-[90px]"
                >
                  <div>
          ...
          <div className="lg:col-span-4 space-y-4">
            <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl p-6 shadow-sm flex flex-col justify-between min-h-[380px]">
              <div>
                <div className="flex justify-between items-center mb-4 pb-3 border-b border-slate-100 dark:border-slate-700">
                  <div>
                    <h4 className="font-black text-sm text-slate-900 dark:text-slate-50">Quick Cart</h4>
                ...
                <div className="space-y-2 max-h-56 overflow-auto custom-scrollbar">
                  {Object.entries(quickCart).map(([name, qty]) => (
                    <div key={name} className="flex justify-between items-center text-xs p-2 rounded-xl bg-slate-50 dark:bg-slate-900">
