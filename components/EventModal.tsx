...
const EventModal: React.FC<EventModalProps> = ({ 
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-slate-900/60 backdrop-blur-xl border-white/10 shadow-sm  rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-200 border border-white/5 ">
        <div className="px-6 py-4 border-b border-white/5  flex justify-between items-center bg-slate-900/60 backdrop-blur-xl border-white/10 shadow-sm ">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            ...
            <input 
              type="text" 
              required
              className="w-full px-3 py-2 border border-white/10  bg-slate-900/60 backdrop-blur-xl border-white/10 shadow-sm  text-slate-50  rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input 
                type="datetime-local" 
                required
                className="w-full px-3 py-2 border border-white/10  bg-slate-900/60 backdrop-blur-xl border-white/10 shadow-sm  text-slate-50  rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none"
                value={start}
                ...
                <button
                  ...
                  className={`flex items-center space-x-2 px-3 py-1.5 rounded-full text-sm border transition-colors ${
                    attendees.includes(member.id) 
                      ...
                      : 'bg-slate-900/60 backdrop-blur-xl border-white/10 shadow-sm  border-white/10  text-slate-300  hover:bg-slate-900/60 backdrop-blur-xl border-white/10 shadow-sm dark:hover:bg-slate-700'
                  }`}
                >
          ...
          <div className="flex items-center justify-between pt-4 mt-4 border-t border-white/5 ">
            ...
            <div className="flex space-x-3">
              <button 
                type="button" 
                onClick={onClose}
                className="px-4 py-2 text-slate-200  font-medium hover:bg-white/10 transition-colors rounded-lg transition-colors"
              >
                Cancel
              </button>
              <button 
