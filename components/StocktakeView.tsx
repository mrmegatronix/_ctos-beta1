    ...
    <div className="flex-1 flex flex-col h-full bg-white dark:bg-slate-900 overflow-hidden">
      <div className="p-6 border-b border-gray-200 dark:border-slate-700 flex justify-between items-center bg-gray-50 dark:bg-slate-800/50">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">Perform physical inventory counts and record variances.</p>
        </div>
        <div>
          {!isActive ? (
            <button onClick={handleStart} className="flex items-center space-x-2 bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2.5 rounded-lg shadow-sm font-medium transition-colors">
              <Play className="w-4 h-4" /> <span>Start Stocktake</span>
            </button>
          ) : (
            <div className="flex space-x-3">
               <button onClick={() => setIsActive(false)} className="px-4 py-2 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-slate-700 rounded-lg transition-colors font-medium">Cancel</button>
               <button onClick={handleCommit} className="flex items-center space-x-2 bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-2 rounded-lg shadow-sm font-medium transition-colors">
      ...
      <div className="p-6">
        <div className="relative max-w-md mb-6">
          ...
          <input 
            ...
            className="w-full pl-10 pr-4 py-2 border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none"
          />
        </div>
                ...
                return (
                  <tr key={item.id} className={`hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors ${item.isDemo ? 'demo-highlight' : ''}`}>
                    ...
                    <td className="px-6 py-4 text-gray-600 dark:text-gray-300">
                      <span className="px-2 py-1 bg-gray-100 dark:bg-slate-700 rounded text-xs">{item.category}</span>
                    </td>
                    ...
                    <td className="px-6 py-4 text-center">
                      <input 
                        ...
                        className="w-24 text-center px-3 py-1.5 border border-gray-300 dark:border-slate-600 rounded-md bg-white dark:bg-slate-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none disabled:opacity-50"
                      />
                    </td>
