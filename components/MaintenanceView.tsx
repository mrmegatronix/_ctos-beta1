          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="flex items-end justify-end space-x-3 pt-2">
              <button
                ...
                className="px-4 py-2.5 text-sm font-medium text-slate-300 hover:bg-white/10 transition-colors rounded-xl transition-colors"
              >
                Cancel
              </button>
              <button
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {pendingTasks.map(task => {
              ...
              return (
                <div
                  key={task.id}
                  className="bg-slate-900/60 backdrop-blur-xl border border-white/10 rounded-2xl p-5 shadow-sm hover:shadow-md transition-shadow flex flex-col justify-between"
                >
                  <div>
            ...
            {pendingTasks.length === 0 && (
              <div className="col-span-full p-12 text-center bg-slate-900/60 backdrop-blur-xl rounded-2xl border border-dashed border-white/10">
                ...
                <p className="text-xs text-slate-400 mt-1">No active maintenance work orders.</p>
              </div>
            )}
          </div>
        </section>
